...
def parse_name(name=None):
    ...
    return re.sub('\s+', ' ', name).strip().title()
...
class SportsGrantSpider(Spider):
    ...
    def parse(self, response):
        for sel in response.xpath('//table/tbody/tr'):
            loader = SportsGrantItemLoader(SportsGrantItem(), selector=sel, response=response)
...
if os.path.isfile(OUTPUT_FILENAME):
    ...
    print("Removed file %s".format(OUTPUT_FILENAME))
